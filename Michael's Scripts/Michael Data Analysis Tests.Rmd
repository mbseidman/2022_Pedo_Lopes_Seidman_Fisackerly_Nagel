...
ggplot(merged,
       aes(x = Age, y = Likes, color = `Domestic/International`)) +
  geom_point(na.rm = TRUE) +
  stat_smooth(method = "lm", formula = y ~ x, se = TRUE, na.rm = TRUE, orientation = "x") +
  scale_x_continuous(limits = c(min(merged$'Age'), max(merged$'Age'))) +
  scale_y_continuous(limits = c(0, max(merged$'Likes'))) +
  labs(x = "Age", y = "Likes",
    subtitle = "") +
  theme(legend.position = "right") +
  ggtitle("The Relationship of Age and Likes")
...
```{r echo=FALSE}
#Personal Life as Predictor of Likes
ggplot(merged,
      aes(x = Step.1=="Personal Life", y = Likes, color = `Domestic/International`)) +
  geom_point(na.rm = TRUE) +
  stat_smooth(method = "lm", formula = y ~ x, na.rm = TRUE) +
    theme(legend.position = "right") +
  facet_wrap(vars(`Domestic/International`)) +
  labs(x = "Personal Life", y = "Likes",
    subtitle = "") +
  ggtitle("The Relationship of Step 1 and Likes")

#Business Life as Predictor of Likes
ggplot(merged,
      aes(x = Step.1=="Business Life", y = Likes, color = `Domestic/International`)) +
  geom_point(na.rm = TRUE) +
  stat_smooth(method = "lm", formula = y ~ x, se = TRUE, na.rm = TRUE) +
    theme(legend.position = "right") +
  facet_wrap(vars(`Domestic/International`)) +
  labs(x = "Business Life", y = "Likes",
    subtitle = "") +
  ggtitle("The Relationship of Step 1 and Likes")
...
```{r echo=FALSE}
ggplot(merged,
       aes(x = `Weighted GPA`, y = Optimism_Score, color = `Discipline`)) +
  geom_point(na.rm = TRUE) +
  stat_smooth(method = "lm", formula = y ~ x, se = TRUE, na.rm = TRUE) +
  facet_wrap(vars(`Discipline`)) +
  theme(legend.position = "none") +
  labs(x = "Weighted Senior GPA", y = "Optimism About Future Employment",
    subtitle = "Faceted by Academic Discipline") +
  ggtitle("The Relationship of Senior Year GPA and Employment Optimism")
  

ggplot(merged,
       aes(x = Social.Cause, y = Likes)) +
  geom_point(na.rm = TRUE) +
  stat_smooth(method = "lm", formula = y ~ x, se = TRUE, na.rm = TRUE) +
  theme(legend.position = "none") +
  labs(x = "Social Cause", y = "Likes",
    subtitle = "") +
  ggtitle("The Relationship of Social Cause and Likes")

ggplot(merged,
       aes(x = 'Current WNBA Team (or Last)', y = Likes)) +
  geom_point(na.rm = TRUE) +
  stat_smooth(method = "lm", formula = y ~ x, se = TRUE, na.rm = TRUE) +
  theme(legend.position = "none") +
  labs(x = "Age", y = "Likes",
    subtitle = "") +
  ggtitle("The Relationship of Ages and Likes")
